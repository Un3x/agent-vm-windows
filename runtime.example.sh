...
#
# WINDOWS / WSL2 NOTE: "~/.agent-vm/runtime.sh" is the home directory INSIDE the
# WSL distro, not on Windows. Create it from a shell in the distro:
#   agent-vm shell
#   mkdir -p ~/.agent-vm && $EDITOR ~/.agent-vm/runtime.sh && chmod +x ~/.agent-vm/runtime.sh
#
# To get started (macOS/Linux):
